...
class SkyProvider (BaseProvider):
    def sky_mac(self):
        return self.hexify(text="^^^^^^^^^^^^")

    def sky_deviceid(self):
        return self.hexify(text="^^^^^^^^^^^^^^^^", upper=True)

    def sky_vcn(self):
        return self.numerify(text="#########")

    def sky_version(self):
        return self.random_element(elements=OrderedDict([("Q120.002.13.00L",0.05), ("Q130.000.27.04L",0.41), ("Q140.000.35.00L",0.02),("Q140.000.37.00L",0.52)]))

    def sky_platform(self):
        return self.random_element(elements=OrderedDict([("eu-q-falconv2-uk",0.05), ("eu-q-titan-uk",0.06), ("bskyb-xwing412",0.10),("bskyb-falcon-d1-uhd",0.03),("bskyb-falcon-d1",0.02),("bskyb-mr412",0.74)]))

    def sky_component(self):
        return self.random_element(elements=OrderedDict([("com.bskyb.epgui",0.96), ("SKPSSP",0.04)]))

    def sky_pem(self):
        return self.random_element(elements=OrderedDict([("SVCLISTUNAVAILABLE",0.35),
            ("BERR",0.18),
            ("HDCP",0.09),
            ("MRCONNERR",0.07),
            ("MRWATCHLIVEERROR",0.07),
            ("NSS",0.06),
            ("Suspicious_reboot",0.04),
            ("SUBSCRIBER_DSU_UPGRADE",0.03),
            ("PDLERROR",0.03),
            ("ALERT_VOD_FAILED_MESSAGE",0.03),
            ("MR_SETUP_NO_GATEWAY",0.02),
            ("NOTAVAIL",0.02)]))
...
class DataGenerator(object):
    ...
    def get_faker_schema(self, fields=None):
        ...
        special_map = {
            ...
            'mac': 'sky_mac',
            'deviceid': 'sky_deviceid',
            'vcn': 'sky_vcn',
            'version': 'sky_version',
            'platform': 'sky_platform',
...
class FakeRowGen(beam.DoFn):
    ...
    def generate_fake(self, fschema=None, key_dict=None):
        ...
        fake = Faker('en_GB')
        fake.add_provider(SkyProvider)

        schema_faker = FakerSchema(faker=fake)
